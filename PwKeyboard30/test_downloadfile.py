...
def test_download_pdf_file(page:Page):
    page.goto("https://testautomationpractice.blogspot.com/p/download-files_25.html")
    # get path for the file
    absolute_path = Path.cwd() / "test_upload_download"
    print(absolute_path)

    page.on("download",lambda download: download.save_as(absolute_path / "testfileassign.pdf"))

    page.locator("#inputText").fill("welcome")
    page.locator("#generatePdf").click()  # this will generate a link to download file
    page.locator("#pdfDownloadLink").click() # this will download the file
    page.wait_for_timeout(5000)

    if os.path.exists("downloads/testfileassign.pdf"):
        print("File exists")
    else:
        print("File not exist")
